...
async function checkAllowedUser(email) {
    return new Promise((resolve, reject) => {
        const url = `https://apicfmukkjxpwpqhdvos.supabase.co/rest/v1/allowed_users?email=eq.${email}&select=*`;
        const options = {
            method: 'GET',
            headers: {
                'apikey': key,
...
(async () => {
    try {
        const user = await checkAllowedUser('xavier.intriago@gmail.com');
        console.log('Allowed User Entry:', JSON.stringify(user, null, 2));
    } catch (e) {
        console.error(e);
    }
})();
