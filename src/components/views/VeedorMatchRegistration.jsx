...
import { ChevronLeft, Trophy, Shield, Edit3, Check, X, Share2 } from 'lucide-react';
...
const VeedorMatchRegistration = ({ match, onBack, teams, onSaveResult, onStartMatch, onRefresh, user }) => {
    const [events, setEvents] = useState(() => {
        return (match.match_events || []).map(dbEv => {
            ...
            // Map explicitly: double_yellow -> 'Doble Amarilla', red_card -> 'Roja', yellow_card -> 'Amarilla'
            let color = '';
            if (dbEv.event_type === 'red_card') color = 'Roja';
            else if (dbEv.event_type === 'yellow_card') color = 'Amarilla';
            else if (dbEv.event_type === 'double_yellow') color = 'Doble Amarilla';
    ...
    const addEvent = async (type, data, timeOverride) => {
        ...
        let eventTypeForDb = type === 'goal' ? 'goal' : 'note';

        if (type === 'card') {
            if (data.color === 'Amarilla') {
                const hasYellow = events.some(e => e.type === 'card' && e.player.id === data.player.id && (e.color === 'Amarilla' || e.color === 'yellow_card'));
                if (hasYellow) {
                    finalData.color = 'Doble Amarilla';
                    eventTypeForDb = 'double_yellow';
                } else {
                    eventTypeForDb = 'yellow_card';
                }
            } else if (data.color === 'Roja') {
                eventTypeForDb = 'red_card';
            }
        }

        const newEvent = { id: Date.now(), type, ...finalData, timestamp, eventTypeForDb }; // Store generic type for UI, specific for DB logic if needed
    ...
    return (
        <div className="fade-in" style={{ paddingBottom: '30px' }}>
            <div style={{ display: 'flex', gap: '10px', marginBottom: '20px' }}>
                ...
                <button
                    onClick={() => shareMatchOnWhatsApp(match)}
                    style={{
                        width: '56px',
                        padding: '16px',
